...
int nAnonymizeAgricoinmnAmount = 1000;
...
#else
    const char* pszModule = "agricoinmn";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Agricoinmn
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Agricoinmn
    // Mac: ~/Library/Application Support/Agricoinmn
    // Unix: ~/.agricoinmn
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Agricoinmn";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "agricoinmn.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "agcmnd.pid"));
