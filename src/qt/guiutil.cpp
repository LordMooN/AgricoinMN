...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("agricoinmn"))
        return false;
...
{
    // Convert agricoinmn:// to agricoinmn:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("agricoinmn://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "agricoinmn:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Agricoinmn.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Agricoinmn.lnk
...
{
    return GetAutostartDir() / "agricoinmn.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a agricoinmn.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Agricoinmn\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the agricoinmn app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add agricoinmn app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Agricoinmn-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  agricoinmn-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Agricoinmn-Qt"));
