...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAgricoinmnAmount = BitcoinUnits::formatWithUnit(nDisplayUnit, nAnonymizeAgricoinmnAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeAgricoinmnAmount = strAnonymizeAgricoinmnAmount.remove(strAnonymizeAgricoinmnAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAgricoinmnAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAgricoinmnAmount*COIN) nMaxToAnonymize = nAnonymizeAgricoinmnAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAgricoinmnAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAgricoinmnAmount));
        strAnonymizeAgricoinmnAmount = strAnonymizeAgricoinmnAmount.remove(strAnonymizeAgricoinmnAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAgricoinmnAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeAgricoinmnAmount == 0){
