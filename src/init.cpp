...
void Shutdown()
{
    ...
    RenameThread("agricoinmn-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: agricoinmn.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: agcmnd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeagricoinmnamount=<n> " + _("Keep N Agricoinmn anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Agricoinmn is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Agricoinmn is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Agricoinmn version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Agricoinmn server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Agricoinmn") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Agricoinmn to complete") << "\n";
    ...
    nAnonymizeAgricoinmnAmount = GetArg("-anonymizeagricoinmnamount", 0);
    if(nAnonymizeAgricoinmnAmount > 999999) nAnonymizeAgricoinmnAmount = 999999;
    if(nAnonymizeAgricoinmnAmount < 2) nAnonymizeAgricoinmnAmount = 2;
    ...
    LogPrintf("Anonymize Agricoinmn Amount %d\n", nAnonymizeAgricoinmnAmount);
